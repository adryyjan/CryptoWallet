...
//  CryptoWallet
//
//  Created by Adrian Mazek on 06/09/2025.
//
        ...
        GeometryReader { geometry in
            Path { path in
                for idx in data.indices {
                    let xPos = Float(geometry.size.width) / CFloat(data.count) * CFloat(idx + 1)
                    
                    let yAx = maxY - minY
                    
                    let yPos = (1  - CGFloat(data[idx] - minY) / yAx) * geometry.size.height
                    
                    if idx == 0 {
                        path.move(to: CGPoint(x: Int(xPos), y: Int(yPos)))
                    }
                    path.addLine(to: CGPoint(x: Int(xPos), y: Int(yPos)))
                }
            }
